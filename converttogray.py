...
def faceDetection(emotion):
    ...
    for f in files:
        ...
        face1 = face_detector1.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, minSize=(5, 5), flags=cv2.CASCADE_SCALE_IMAGE)
        face2 = face_detector2.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, minSize=(5, 5), flags=cv2.CASCADE_SCALE_IMAGE)
        face3 = face_detector3.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, minSize=(5, 5), flags=cv2.CASCADE_SCALE_IMAGE)
        face4 = face_detector4.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10, minSize=(5, 5), flags=cv2.CASCADE_SCALE_IMAGE)
        ...
        for (x, y, w, h) in facefeatures: #get coordinates and size of rectangle containing face
            ...
            try:
                ...
                cv2.imwrite("D:\\FERA\\final_ dataset\\%s\\%s.jpg" %(emotion, filenumber), out) #Write image
                print ("Image Saved")
